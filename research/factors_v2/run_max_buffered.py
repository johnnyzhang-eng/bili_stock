"""
MAX Factor with Buffered Rebalancing
=====================================

Unbuffered MAX has IC=0.0378/ICIR=0.281 (stronger than low_vol) but
turnover=58.1%/period makes net_top_ann=-2.21%. Test hysteresis buffering
to cut turnover while preserving the alpha core.

Same approach as run_low_vol_buffered.py:
  enter_q = 0.80 (top 20%)
  keep_q  ∈ {0.80, 0.70, 0.60, 0.50}   keep_q=0.80 ≡ baseline (no buffer)

Run:
    python research/factors_v2/run_max_buffered.py
"""
...
def main():
    ...
    print(f"Building MAX factor (window={MAX_WINDOW}) ...", flush=True)
    stock_dir = os.path.join(ROOT, "data", "stock_data")
    start = panel["date"].min().strftime("%Y-%m-%d")
    end   = panel["date"].max().strftime("%Y-%m-%d")
    mx = build_max_factor(stock_dir, start_date=start, end_date=end, window=MAX_WINDOW)
    ...
    rows = []
    print("\n" + "=" * 82)
    print(f"MAX factor | enter_q={ENTER_Q}, hold_step={HOLD_STEP}, window={MAX_WINDOW}")
    ...
    for keep_q in [0.80, 0.70, 0.60, 0.50]:
        r = _buffered_backtest(panel, "max_z", HOLD_STEP, enter_q=ENTER_Q, keep_q=keep_q)
        if not r:
            print(f"{keep_q:>7.2f} | (no result)")
            continue
    ...
    # Compare with low_vol reference numbers from prior run
    print("\nReference (low_vol buffered, keep_q=0.70): net_top ≈ 19.16% (pre-survivorship)")
    print("Reference (low_vol unbuffered baseline):  net_top ≈  2.40% (this IC runner's metric)")

    out_dir = os.path.join(ROOT, "research", "factors_v2", "output")
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, "max_buffered.csv")
