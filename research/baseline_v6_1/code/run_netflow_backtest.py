"""
Net Flow Backtest: Count vs Net_Flow factor with choppy_fix_B params
====================================================================
Compares the production signal (count) against net_flow (buy-sell) in a
full backtest with choppy_fix_B parameters.

Experiments:
  A) Baseline count + go-flat choppy (current production)
  B) Net flow signal + go-flat choppy
  C) Net flow signal + NO go-flat (trade all days — choppy IC is positive)
  D) Net flow signal + original choppy_loss_scale=0.50

If net_flow has positive choppy IC (+0.0052), experiment C should outperform
by trading profitably in the 30% of days currently skipped.

Run: python research/baseline_v6_1/code/run_netflow_backtest.py
"""
...
def main():
    ...
    experiments = [
        {
            ...
            "desc": "Count signal + go-flat choppy (current production)",
            "panel": panel_count,
            "px_map": px_map_count,
            "risk": {**RISK_BASE, "choppy_loss_scale": 0.0},
        },
        {
            "label": "B_netflow_goflat",
            "desc": "Net flow + go-flat choppy",
            "panel": panel_nf,
            "px_map": px_map_nf,
            "risk": {**RISK_BASE, "choppy_loss_scale": 0.0},
        },
        ...
        {
            "label": "D_netflow_halfchoppy",
            "desc": "Net flow + original choppy_loss_scale=0.50",
            "panel": panel_nf,
            "px_map": px_map_nf,
            "risk": {**RISK_BASE, "choppy_loss_scale": 0.50},
        },
    ]
    ...
    for exp in experiments:
        ...
        print(
            ...
            f"  sharpe={sharpe:.3f}  ({time.time()-t2:.0f}s)",
            flush=True,
        )
    ...
    grid_path = os.path.join(out_dir, "netflow_backtest_grid_2010_2025.csv")
